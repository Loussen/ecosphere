...
if($_POST) // Add && edit
{
    ...
    $date = mktime($hour,$minute,00,$month,$day,$year);
    ...
    while($row=mysqli_fetch_assoc($sql))
    {
        ...
        $customer="customer_".$row["id"]; $customer=mysqli_real_escape_string($db,htmlspecialchars($$customer));
        $category="category_".$row["id"]; $category=mysqli_real_escape_string($db,htmlspecialchars($$category));
        $status="status_".$row["id"]; $status=mysqli_real_escape_string($db,htmlspecialchars($$status));
        $demo="demo_".$row["id"]; $demo=mysqli_real_escape_string($db,htmlspecialchars($$demo));
        $tags="tags_".$row["id"]; $tags=mysqli_real_escape_string($db,htmlspecialchars($$tags));
        ...
        {
            mysqli_query($db,"update $do set title='$title',customer='$customer',category='$category',`date`='$date',status='$status',demo='$demo',tags='$tags',text='$text',updated_at='$time' where lang_id='$row[id]' $add_where");
        }
        else
        {
            mysqli_query($db,"insert into $do values (0,'$title','$customer','$category','$date','$status','$demo','$tags','$text','','$last_order','$active', '$row[id]', '$auto_id', '$time',0) ");
        }
    }
    ...
    if($image_tmp!="")
    {
        ...
        if($image_access==true)
        {
            ...
            if($edit>0)
            {
                ...
                @unlink("../images/projects/".$old_image_name['image_name']);
            }

            move_uploaded_file($image_tmp,'../images/projects/'.$image_upload_name);

            compress('../images/projects/'.$image_upload_name, '../images/projects/'.$image_upload_name, 80);
...
if($delete>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where auto_id='$delete' "))>0)
{
    ...
    @unlink('../images/projects/'.$data["image_name"]);
...
<div class="onecolumn">
    ...
    <div class="content">
        ...
        <form action="index.php?do=<?php echo $do; ?>&page=<?php echo $page; if($edit>0) echo '&edit='.$edit; ?>" method="post" id="form_login" name="form_login" enctype="multipart/form-data">
            ...
            <?php
                ...
                while($row=mysqli_fetch_assoc($sql))
                {
                    ...
                    echo 'Title : <br />
                      ...
                      <br /><br />
                      Customer : <br />
                      <input type="text" name="customer_'.$row["id"].'" value="'.$information["customer"].'" style="width:800px" />
                      <br /><br />
                      Category : <br />
                      <input type="text" name="category_'.$row["id"].'" value="'.$information["category"].'" style="width:800px" />
                      <br /><br />
                      Status : <br />
            ...
            {
                $year = date("Y",$information['date']);
                $month = date("m",$information['date']);
                $day = date("d",$information['date']);
                $hour = date("H",$information['date']);
                $minute = date("i",$information['date']);
            }
            else
            {
            ...
            {
                $image='<ul class="media_photos" style="margin-left:450px;margin-top:-25px; margin-right: 40px;"><li><a rel="slide" href="../images/projects/'.$information["image_name"].'?rand='.rand(0,10000).'" title="">
                        Current image : <img src="../images/projects/'.$information["image_name"].'?rand='.rand(0,10000).'" alt="" width="120" height="60" /></a>
                        <br class="clear" />
        ...
        while($row=mysqli_fetch_assoc($sql))
        {
            echo '<tr>
					...
					<td><ul class="media_photos" style="padding:0;margin-top:0px;margin-bottom:-40px"><li><a rel="slide" href="../images/projects/'.$row["image_name"].'" title=""><img src="../images/projects/'.$row["image_name"].'?rand='.rand(0,10000).'" width="100" height="60"></a></li></ul></td>
					<td>
