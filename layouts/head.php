<!--<base href="--><?//=SITE_PATH?><!--">-->
<meta charset="utf-8">
<title>Ecosphere - Industries and Factories</title>
<!-- Stylesheets -->
<link href="<?=SITE_PATH?>/css/bootstrap.css" rel="stylesheet">
<link href="<?=SITE_PATH?>/plugins/revolution/css/settings.css" rel="stylesheet" type="text/css">
<!-- REVOLUTION SETTINGS STYLES -->
<link href="<?=SITE_PATH?>/plugins/revolution/css/layers.css" rel="stylesheet" type="text/css"><!-- REVOLUTION LAYERS STYLES -->
<link href="<?=SITE_PATH?>/plugins/revolution/css/navigation.css" rel="stylesheet" type="text/css">
<!-- REVOLUTION NAVIGATION STYLES -->
<link href="<?=SITE_PATH?>/css/style.css" rel="stylesheet">
<link href="<?=SITE_PATH?>/css/responsive.css" rel="stylesheet">

<!--Color Switcher Mockup-->
<link href="<?=SITE_PATH?>/css/color-switcher-design.css" rel="stylesheet">

<!--Color Themes-->
<link id="theme-color-file" href="<?=SITE_PATH?>/css/color-themes/default-theme.css" rel="stylesheet">

<link rel="shortcut icon" href="<?=SITE_PATH?>/images/favicon-eco.png" type="image/x-icon">
<link rel="icon" href="<?=SITE_PATH?>/images/favicon-eco.png" type="image/x-icon">
<!-- Responsive -->
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0">